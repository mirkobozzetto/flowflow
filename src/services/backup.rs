...
fn snapshot_device_id(snapshot: &Path) -> Result<String, String> {
    let conn = open_read_only(snapshot)?;
    conn.query_row(
...
pub async fn export_archive(
    ...
    let snapshot = create_scrubbed_snapshot(&crate::db::db_path(), &staging)?;
    let audio_dir = PathBuf::from(crate::services::audio::output_dir());
...
pub fn validate_archive_at(
    ...
    let app_schema = crate::db::current_schema_version();
...
pub fn default_restore_paths() -> RestorePaths {
    RestorePaths {
        ...
        bak: restore_bak_dir(),
        data_db: crate::db::raw_db_path(),
        audio_dir: PathBuf::from(crate::services::audio::output_dir()),
        vectordb_dir: PathBuf::from(crate::services::vectordb::vectordb_path()),
...
mod import_tests {
    ...
    fn newer_schema_archive_is_refused() {
        ...
        manifest.schema_version = crate::db::current_schema_version() + 1;
