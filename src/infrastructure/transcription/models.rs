use crate::application::constants::{WhisperCatalogEntry, WHISPER_CATALOG};
            ...
            match result {
                Ok(()) => {
                    if let Ok(db) = crate::infrastructure::persistence::Database::open() {
                        let key = crate::infrastructure::persistence::settings_repo::WHISPER_MODEL_KEY;
                        let has_active = db
                            .get_setting(key)
                            .filter(|v| !v.is_empty())
                            .is_some();
...
pub fn models_dir() -> PathBuf {
    #[cfg(target_os = "ios")]
    let base = crate::infrastructure::platform::ios::documents_dir();
    #[cfg(not(target_os = "ios"))]
    let base = crate::infrastructure::persistence::desktop_data_dir();
    base.join(crate::application::constants::WHISPER_MODELS_SUBDIR)
}
