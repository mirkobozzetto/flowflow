...
pub fn ChatMenu(props: ChatMenuProps) -> Element {
    ...
    let menu_context =
        format!("{}:{instance}", conversation_id().unwrap_or_default());
    use_effect(move || {
        if !(app.show_chat_menu)() {
            renaming.set(false);
        ...
        div {
            ...
            div { class: "absolute right-4 top-1 {kit::MENU_PANEL}",
                if renaming() {
                    ...
                    button {
                        class: kit::MENU_ITEM,
                        "data-native-action": "rename",
                        "data-native-symbol": "pencil",
                        disabled: !has_conversation,
                        onclick: move |_| {
                            if let Some(ref cid) = conversation_id() {
