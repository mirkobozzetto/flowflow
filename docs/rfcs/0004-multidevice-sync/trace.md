...
| T06 | C6 | todo | `services/sync/transport.rs` | - | SPIKE snow XXpsk3. Prochaine passe. |
| T08 | C7 | todo | `services/sync/peers.rs`, `ui/` | - | SPIKE QR/IP. Prochaine passe. |
...
- 2026-06-09: passe 1 (T01-T05) shipped. 7 fichiers src modifiés + 1 nouveau (sync_meta.rs) + Cargo.toml (feature functions) + tests/sync_meta_test.rs (12 tests). Non commité. En attente: build + test device par Mirko, puis passe 2 (spikes T06/T08).
